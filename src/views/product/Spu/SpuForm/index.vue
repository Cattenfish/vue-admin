...
export default {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      ...
      //收集照片墙图片的数据
      //对于已有的图片（照片墙中显示的图片，有name和url字段），因为照片墙显示的数据务必要有这两个字段
    ...
    addSaleAttrValue(row) {
      //点击销售属性值当中的添加按钮的时候，需要有button变为input通过当前销售属性的inputVisible控制
      //挂载在销售属性上的响应式数据inputVisible 控制buuton与input切换
    ...
    cancel() {
      ...
      //this.$options可以获取配置对象，配置对象data函数执行，返回响应式数据为空的
      Object.assign(this._data, this.$options.data());
    },
  },
  ...
  computed: {
    //计算出还未选择的销售属性
    unSelectSaleAttr() {
      //整个平台销售属性为三个saleAttrList 颜色 尺寸 版本
